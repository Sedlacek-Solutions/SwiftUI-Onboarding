...
public struct HeroWelcomeScreen {
  public struct Configuration {
    ...
    public let languageSelectionDelay: Duration
    ...
    public let topContentHeight: CGFloat
    public let topContentSpacing: CGFloat
    public let bottomContentTopPadding: CGFloat
    ...
    public init<HeroContent: View>(
      ...
      self.init(
        ...
        topContentHeight: 0,
        topContentSpacing: Layout.verticalSpacing,
        bottomContentTopPadding: Layout.bottomContentTopPadding,
        topContent: { EmptyView() },
        heroContent: heroContent
      )
    }

    public init<TopContent: View, HeroContent: View>(
      accentColor: Color = .blue,
      title: LocalizedStringKey,
      ctaTitle: LocalizedStringKey = "Get Started",
      accountPrompt: LocalizedStringKey = "Already have an account?",
      ...
      self.defaultLanguageIdentifier =
        defaultLanguageIdentifier
        ?? languageOptions.first?.identifier
        ?? Locale.current.identifier
      self.languageSelectionDelay = languageSelectionDelay
      ...
      self.bottomContentTopPadding = bottomContentTopPadding
  ...
  @State private var pendingLanguageSelection: OnboardingLanguageOption?
  @State private var languageSelectionTask: Task<Void, Never>?

  private enum Layout {
    static let horizontalPadding: CGFloat = 24
    static let topPadding: CGFloat = 18
    static let verticalSpacing: CGFloat = 28
    static let bottomContentTopPadding: CGFloat = 18
  ...
  private func queueLanguageSelection(_ language: OnboardingLanguageOption) {
    pendingLanguageSelection = language
  }

  private func applyPendingLanguageSelection() {
    guard let language = pendingLanguageSelection else { return }
    pendingLanguageSelection = nil
    languageSelectionTask?.cancel()
    languageSelectionTask = Task { @MainActor in
      try? await Task.sleep(for: config.languageSelectionDelay)
      guard !Task.isCancelled else { return }
      selectedLanguageIdentifier = language.identifier
      config.languageSelectionAction(language)
    }
  }

  private func cancelPendingLanguageSelection() {
    languageSelectionTask?.cancel()
...
@MainActor
extension HeroWelcomeScreen.Configuration {
...
extension HeroWelcomeScreen: View {
  public var body: some View {
    NavigationStack {
      GeometryReader { geometry in
        VStack(spacing: config.topContentSpacing) {
          config.topContent
            .frame(height: config.topContentHeight)
      ...
      .sheet(
        ...
        onDismiss: applyPendingLanguageSelection
      ) {
        HeroLanguagePickerSheet(
          accentColor: config.accentColor,
          languageOptions: config.languageOptions,
          selectedLanguageIdentifier: selectedLanguageIdentifier,
          languageSelectionAction: queueLanguageSelection
        )
        .environment(\.locale, selectedLocale)
      }
      .onAppear(perform: onAppear)
      .onDisappear(perform: cancelPendingLanguageSelection)
    }
  ...
  private func heroSection(in availableSize: CGSize) -> some View {
    ...
    let contentHeight = max(
      0,
      availableSize.height - Layout.topPadding - config.topContentHeight - config.topContentSpacing
    )
  ...
  private func bottomSection() -> some View {
    HeroWelcomeBottomSection(
      ...
      topPadding: config.bottomContentTopPadding,
...
@MainActor
extension HeroWelcomeScreen.Configuration {
  fileprivate static let scalableImagePreview = Self(
    accentColor: .blue,
